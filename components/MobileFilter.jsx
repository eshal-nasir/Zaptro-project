    ...
    </div>
{
    ...
    <div className='bg-gray-100 p-2 md:hidden'>
     ...
     <div className='flex flex-col gap-2 mt-3 '>
      {
          categoryOnlyData?.map((item, index) => {
                    ...
                    />
                    <button className='cursor-pointor uppercase'>{item}</button>
                    </div>
                })
      }
     </div>
     ...
     <h1 className='mt-5 font-semibold text-xl mb-3'>Brand</h1>
      ...
      <input type="range"  min= '0' max='5000' name="" id="" value={priceRange[1]} onChange={(e)=>setPriceRange([priceRange[0], Number(e.target.value)])} className='transition-all w-[300px]' />
         </div>  
         <button className='bg-red-500 text-white rounded-md px-3 py-1 mt-5 cursor-pointer' onClick={()=>{setSearch(''); setCategory('All'); setBrand('All'); setPriceRange([0,5000]) ; setOpenFilter(false)
